  ...
  <div class="setting">
    ...
    >
      ...
      <div class="right-side">
        <span v-if="item.inputType == 'input'">{{ item.inputData }}</span>
        <div
          v-else
          class="switch-container"
          @click="switchValues[i] = !switchValues[i]"
        >
          <div class="switch" @click="toggleSwitch(item)">
            <input type="checkbox" :checked="item.inputData" />
            <span class="switch-slider"></span>
          </div>
          <div class="switch-right">
            <div class="switch-state">
              <!-- {{  switchValues[i] ? "Açık" : "Kapalı" }} -->
            </div>
          </div>
        </div>
      </div>
...
const switchValues = ref([false, false, false]);
const toggleSwitch = (item: any) => {
  if (item.inputData) {
    item.inputData = false;
  } else {
    item.inputData = true;
  }
