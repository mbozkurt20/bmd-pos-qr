...
import { changeOrderStatusAPI, onlineStore } from "../../../store/online";
import { givedIcon, inTheRoadIcon, preparingIcon } from "../../../constants/icons";
  ...
  <div class="d-flex mt-4" style="justify-content: space-between;">
    <div class="orders-right-state">
      <template v-for="(item, idx) in statuses" :key="idx">
        <div @click="item.isShow && item.status !== order.order_status.id ? changeOrderStatusAPI(item.status) : null"
          :class="['orders-right-state-item d-flex gap-2', {
            'orders-right-state-item-active': item.status === order.order_status.id
          }]" :style="`
