<template>
  <div @click="selectOrder" class="orders-item" :class="{'orders-item-active': getIsSelected}">
    <div class="orders-item-left">
      ...
      <div class="orders-item-texts">
        ...
        <div class="orders-item-method" style="font-size: 12px">
    ...
    <div class="orders-item-right">
      ...
      <div class="orders-item-state" :class="'orders-item-state-' + order.order_status.id" v-if="order.order_status.id === 1">
        <ion-icon name="time-outline" style="font-size: 18px" />

        Onay Bekliyor
      </div>

      <div class="orders-item-state" :class="'orders-item-state-' + order.order_status.id" v-if="order.order_status.id === 2">
      ...
      <div class="orders-item-state" :class="'orders-item-state-' + order.order_status.id" v-if="order.order_status.id === 3">
        <svg xmlns="http://www.w3.org/2000/svg" class="ionicon" viewBox="0 0 512 512">
          <path d="M388 288a76 76 0 1076 76 76.24 76.24 0 00-76-76zM124 288a76 76 0 1076 76 76.24 76.24 0 00-76-76z"
      ...
      <div class="orders-item-state" :class="'orders-item-state-' + order.order_status.id" v-if="order.order_status.id === 4">
        <svg xmlns="http://www.w3.org/2000/svg" class="ionicon" viewBox="0 0 512 512">
      ...
      <div class="orders-item-state" :class="'orders-item-state-' + order.order_status.id" v-if="order.order_status.id === 5">
        <svg xmlns="http://www.w3.org/2000/svg" class="ionicon" viewBox="0 0 512 512">
          <path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="32"
...
import { onlineStore , setOrder} from "../../store/online";

const props = defineProps(['order'])

const getIsSelected = computed(() => props.order.id === onlineStore.order?.id);
...
const getTotal = computed(() => {
  return props.order.order_items.reduce((acc, order) => {
...
<style src="./POrdersItem.scss" lang="scss"/>